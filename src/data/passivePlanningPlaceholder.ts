...
/** Passive / period_summary gap copy — must not promise planning buttons before active_planning. */
export function resolvePassiveGapPlaceholderText(age: number): { title: string; text: string } {
  if (age <= 2) {
    return { title: '岁月静流', text: '这一季你在家人怀抱与啼哭声中度过，尚不知江湖为何物。' };
  }
  if (age <= 4) {
  ...
  if (age <= 7) {
    return {
      title: '童年时光',
      text: '这一季在家中庭院与亲人、玩伴身边度过，日子平淡而安稳。',
    };
  }
  return {
    title: '季节更迭',
    text: '这一季光阴寻常流过，未见更多江湖波澜。',
  };
}
