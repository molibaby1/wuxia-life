...
import { EventExecutor } from '../core/EventExecutor';
import { EventLoader } from '../core/EventLoader';
import { createDefaultPlayerLifeStates } from '../data/life/lifeStates';
import { resolveChildhoodActionPalette } from '../p16/childhoodAgency';
...
export async function runRealisticNewGameMerchantScenario(): Promise<MerchantScenarioProof> {
  ...
  const afterChoice = await executor.executeEffects(choice.effects ?? [], engine.getGameState());
  engine.applyGameState(afterChoice);
