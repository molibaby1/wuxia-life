...
- 该边界是 **abnormal-safety hard boundary**，不是 ordinary execution budget，也不是“任务应当在此时长内完成”的质量目标；正常 Participant execution 应在该 boundary 内自然完成，不应被提前预算化或人为截断。
...
- 不自动新增 Person/Event，不以“补充篇幅”为理由扩写，不绕过 Human Approval，不把 `READY_FOR_FORMAL_TASK` 或 Agent 输出当作 implementation authorization。
