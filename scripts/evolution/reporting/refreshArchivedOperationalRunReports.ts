...
export async function refreshArchivedOperationalRunReports(input: {
  ...
  for (const entry of entries.filter(item => item.isDirectory()).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const markdown = renderOperationalRunReportMarkdownFromReport({
      ...
      ...(
        report.schemaVersion === 'auto-evolution-operational-run-report-v4'
        || report.schemaVersion === 'auto-evolution-operational-run-report-v5'
        || report.schemaVersion === 'auto-evolution-operational-run-report-v6'
          ? { workspaceProvenance: report.workspaceProvenance }
          : {}
      ),
    });
