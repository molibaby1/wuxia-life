...
function v7(reportId: string, logicalSessionId: string, hostSliceId: string, createdAt: string, candidates: unknown[] = []) {
  return {
    ...
    sessionExecution: {
      ...
      sourceEpochs: [{ sourceEpochRef: 'source-epoch-000001', sourceRunRef: 'source-run-000001', poolRef: null, poolStatus: 'PROCESSING', lifecycle: 'ANALYSIS_PENDING', candidateCounts: { total: 0, pending: 0, active: 0, completed: 0, superseded: 0, interrupted: 0 }, dispositionCounts: {} }],
      hostSlices: [{ hostSliceId, startedAt: createdAt, endedAt: createdAt, participantJobs: 0, state: 'PAUSED', reason: 'HOST_SLICE_BUDGET' }],
