...
function testBridgeGateFlags(): void {
  ...
  assert(flags.tavern_guest_network === true, 'tavern_guest_network should be true');
  assert(flags.ally_network === true, 'ally_network should be true');
  assert(flags.tavern_embrace_network === true, 'tavern_embrace_network should be true');
  assert(flags.tavern_take_referral === true, 'tavern_take_referral should be true');
  assert(flags.route_wealth_committed === true, 'route_wealth_committed should be true');
  assert(flags.tavern_merchant_bridge_crossed === true, 'tavern_merchant_bridge_crossed should be true');
}

function testBridgeGateRequiresTakeReferral(): void {
  const noReferral = makeState(30, {
    origin_tavern_hand: true,
    tavern_guest_network: true,
    ally_network: true,
    tavern_embrace_network: true,
  ...
  const flags = noReferral.flags ?? {};
  assert(flags.tavern_take_referral !== true, 'bridge should require take_referral');
  assert(flags.tavern_merchant_bridge_crossed !== true, 'bridge should require tavern_merchant_bridge_crossed');
}
...
function testBridgeCurrentGoalExpression(): void {
  ...
  assert(goal.includes('商路') || goal.includes('铺子'), `bridge currentGoal: ${goal}`);
  ...
  assert(midlifeGoal.includes('人脉') || midlifeGoal.includes('常客'), `midlife currentGoal: ${midlifeGoal}`);
...
function testBridgeLifeMemoryExpression(): void {
  ...
  assert(Boolean(memory?.includes('商路') || memory?.includes('铺子')), `bridge lifeMemory: ${memory}`);
  assert(isPlayerVisibleOrdinaryOriginText(memory ?? ''), `raw key in bridge lifeMemory: ${memory}`);

  const referral = makeState(27, {
    origin_tavern_hand: true,
    tavern_midlife_ally_referral: true,
    tavern_take_referral: true,
  });
  const referralMemory = deriveOrdinaryOriginLifeMemory(referral.flags ?? {});
  assert(Boolean(referralMemory?.includes('引荐') || referralMemory?.includes('铺子')), `referral lifeMemory: ${referralMemory}`);
}
...
function testBridgeSummaryExpression(): void {
  ...
  assert(Boolean(midlifeSummary?.includes('酒肆')), `midlife summary: ${midlifeSummary}`);
...
function testBridgeDoesNotBreakOrdinaryOrigin(): void {
  ...
  assert(origin === 'tavern_hand', `bridge should keep ordinary origin: ${origin}`);
}
...
function testNonTavernNoBridge(): void {
...
const MAGNATE_ON_RAMP_EXPRESSION =
  "(flags.has('route_merchant') || flags.has('merchant_childhood_seed_done') || flags.has('p8_route_wealth') || flags.has('apprentice_merchant_bridge_crossed') || flags.has('tavern_merchant_bridge_crossed')) && (flags.has('merchant_caravan_success') || flags.has('merchant_shop_grocery') || flags.has('merchant_shop_weapon') || flags.has('merchant_shop_herb') || flags.has('merchant_wealthy') || flags.has('merchant_chamber_head') || flags.has('apprentice_merchant_bridge_crossed') || flags.has('tavern_merchant_bridge_crossed')) && !flags.has('magnate_on_ramp_done') && !flags.has('orthodox_childhood_seed_done') && !flags.has('demonic_childhood_seed_done')";

const MERCHANT_MIDLIFE_DEBT_EXPRESSION =
  "(flags.has('route_merchant') || flags.has('merchant_childhood_seed_done') || flags.has('merchant_talent') || flags.has('p8_route_wealth') || flags.has('apprentice_merchant_bridge_crossed') || flags.has('tavern_merchant_bridge_crossed')) && (flags.has('merchant_shop_grocery') || flags.has('merchant_shop_weapon') || flags.has('merchant_shop_herb') || flags.has('merchant_shop_failed') || flags.has('merchant_caravan_success') || flags.has('apprentice_merchant_bridge_crossed') || flags.has('tavern_merchant_bridge_crossed')) && !flags.has('merchant_midlife_debt') && !flags.has('orthodox_childhood_seed_done') && !flags.has('demonic_childhood_seed_done')";
...
function testMagnateOnRampGateAcceptsBridgeFlag(): void {
  ...
  assert(result === true, 'magnate_on_ramp gate should accept tavern_merchant_bridge_crossed');
}
...
function testDeclineReferralNoBridge(): void {
  const state = makeState(30, {
    origin_tavern_hand: true,
    tavern_guest_network: true,
    ally_network: true,
    tavern_midlife_ally_referral: true,
    tavern_decline_referral: true,
    ordinary_tavern_midlife_done: true,
  });
  const flags = state.flags ?? {};
  assert(flags.tavern_merchant_bridge_crossed !== true, 'decline_referral should not cross bridge');
  const goal = deriveOrdinaryOriginCurrentGoal(state) ?? '';
  assert(!goal.includes('商路'), `decline referral should not have bridge goal: ${goal}`);
}

function main(): void {
  testBridgeGateFlags();
  testBridgeGateRequiresTakeReferral();
  ...
  testDeclineReferralNoBridge();
  console.log('p59TavernHandBridgeTests: all passed');
}
