...
| `WEALTH_REQUIREMENT` | `merchant.json` 的 `wealth_capacity_at_least`：`merchant_first_shop` / `invest_more`（`modest_savings`）、`hire_elite_guards`（`comfortable_means`）、`merchant_market_monopoly` 入口（`comfortable_means`）、`merchant_official_connection.heavy_bribe`（`wealthy`）、`merchant_chamber_of_commerce`（`comfortable_means`）；仍未迁移的 `money` threshold 如 `identity-merchant.json`、`relationship.json`，以及 downstream merchant consumer（如 `merchant_sect_investment`、merchant endings） | merchant talent → shop → caravan → market → official → intelligence → chamber 竖切已迁移；`moderate_bribe` 等无 Wealth 门槛路径已退役 wallet gate；余额阈值 consumer 仍存在于 identity-merchant、relationship 与下游 sect/ending 等 deferred path |
| `WEALTH_TRANSITION` | `origin.json` 的 `wealth_capacity_set: comfortable_means`；`merchant.json` 的 `wealth_capacity_raise_to modest_savings`（`study_business`）、`wealth_capacity_raise_to comfortable_means`（`escort_personally`）、`wealth_capacity_raise_to wealthy`（`merchant_market_monopoly.monopoly_trade`、`merchant_chamber_of_commerce`）与 `wealth_capacity_set: regional_magnate`（`merchant_wealth_peak`）；`fair_competition` 不改变 Wealth Capacity；legacy money delta 仍在 `origin_merchant_family` / `merchant_wealth_peak`（deferred） | transition 不等于余额加减；raise_to 与 set 已在 merchant 主线多段并存；origin/peak 的 legacy `money +200` 仍属 deferred debt |
