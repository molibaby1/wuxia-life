...
export type RealControlSummary = {
  runId: string;
  passed: boolean;
  failures: string[];
  cases: RealControlCaseResult[];
  /** Soft diagnostics never hard-kill Control. */
  softDiagnosticsDoNotHardKill: true;
};
...
export async function runRealControlCheck(options?: {
  ...
  const outDir = options?.outDir ?? join(options?.outRoot ?? '.tmp/b0', runId);
  ...
  const summary: RealControlSummary = {
    runId,
