...
export function buildModificationWorkParticipantInstructions(): string {
  return [
    ...
    'evidence pack 或 investigationHandoff.evidenceBasis 中可能出现 Experience Semantic Context / Experience Pattern：它们只是描述性玩家可观察语义，不是 hidden runtime state，也不得当作 solution recommendation、quality score、authority 或 permission。',
...
export function buildModificationWorkV2ParticipantInstructions(): string {
  return [
    ...
    'evidenceRefs 必须全部存在于所提供 evidence pack。',
    'evidence pack 或 investigationHandoff.evidenceBasis 中可能出现 Experience Semantic Context / Experience Pattern：它们只是描述性玩家可观察语义，不是 hidden runtime state，也不得当作 solution recommendation、quality score、authority 或 permission；也不得把 evidenceBasis 本身当作 investigationBasisRefs。',
